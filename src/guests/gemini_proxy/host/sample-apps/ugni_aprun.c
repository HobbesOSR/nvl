...
int main (int argc, char *argv[])
{
    ...
    cmd.cookie = alps_info.cookie;
    cmd.instance = alps_info.ptag;
    ...
    if (err < 0) {
        ...
        printf("succes sending ptag, cookie  command to vm\n");
    close(vm_fd);
...
//                GNI_CDM_MODE_ERR_NO_KILL|GNI_CDM_MODE_BTE_SINGLE_CHANNEL,
        ...
        rc=GNI_CdmAttach(cdm_hdl,
                alps_info.device_id,
                (uint32_t*)&alps_info.local_addr, /* ALPS and GNI disagree about the type of local_addr.  cast here. */
                &nic_hdl);
        if (rc!=GNI_RC_SUCCESS) {
