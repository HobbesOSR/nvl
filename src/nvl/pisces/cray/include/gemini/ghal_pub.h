...
#define GHAL_CLDEV_CE_STR		NULL
#define GHAL_CLDEV_DLA_STR		NULL
...
#define _GHAL_FMA_SET_RC(desc_cpy,mode)
...
#define GHAL_FMA_PUT_S          /* PUT_S is not supported on Gemini */
...
#define GHAL_PCI_IOMMU_GET_ADDR(entry) 0

/* Memory Relocation Table */
inline static int ghal_mrt_code_to_size(int code)
{
...
inline static int ghal_mrt_code_to_shift(int code)
{
...
inline static int ghal_mrt_code_to_mask(int code)
{
        int memhndl_mask[]={
                ~(128*1024-1),
                ~(512*1024-1),
...
inline static int ghal_mrt_size_to_code(unsigned long size)
{
	int code;
