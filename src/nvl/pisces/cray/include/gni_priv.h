...
#define GNI_DRV_CODE_REV  0x00b1
#else
#include "gni_priv_svn_rev.h"
#endif
...
#define GNI_DLA_NUM_CONFIG_PARAMS 11
...
typedef struct gni_nic {
        ...
        uint64_t        unused1;
        uint64_t        unused2;
        gni_cq_handle_t dla_cq_hndl;    /* dla event queue */

	/* DON'T ALTER ANYTHING ABOVE, as will break DMAPP/UGNI/SMSG compatibility */
        ...
        uint8_t         cpr_checkpoint; /* in checkpoint mode? */
...
#else
        void                    *ioproc;       /* pointer to allocated fork ioproc */
#endif
        struct mm_struct        *mm;
        pid_t                   pid;
...
extern char gni_nic_assert[1 - 2*!(offsetof(gni_nic_t, dla_cq_hndl) == 136)];
...
typedef struct reqfull_list {
        struct reqfull_list *next;
...
typedef struct gni_ep_smsg_mbox {
        ...
        int32_t             retransmit_count;      /* count of retransmitted message and back credit requests in flight */
...
typedef struct gni_ep {
        ...
        uint16_t            outstanding_tx_reqs;   /* number of outstanding tx reqs associated with this ep */
...
#define UGNI_CQ_BLOCKED_DELAY_INTERVAL_ENV_VAR    "UGNI_CQ_BLOCKED_DELAY_INTERVAL"      /* uGNI environment variable */
...
typedef struct gni_cq {
        ...
        gnii_reqfull_list_t     *reqfull_list;
        uint64_t                kern_cq_descr;   /* kernel agent CQ descriptor */
	...
	/* DON'T ALTER ANYTHING ABOVE, as will break DMAPP/UGNI compatibilty */
        ...
        uint32_t                mem_size;       /* size of the allocated memory */
...
#define GNI_MEMHNDL_FLAG_UNUSED         0x80UL /* Unused */
/* Memory Handle manipulations  */
#define GNI_MEMHNDL_INIT(memhndl) do {memhndl.qword1 = 0; memhndl.qword2 = 0;} while(0)
/* Virtual address [63:12]: QWORD1[51:0] */
#define GNI_MEMHNDL_VA_MASK     0x000FFFFFFFFFFFFF
#define GNI_MEMHNDL_SET_VA(memhndl, value) memhndl.qword1 |= ((value >> 12) & GNI_MEMHNDL_VA_MASK)
#define GNI_MEMHNDL_GET_VA(memhndl) ((memhndl.qword1 & GNI_MEMHNDL_VA_MASK ) << 12)
/* Memory domain handle (4K entries): QWORD1[63:52] */
#define GNI_MEMHNDL_MDH_MASK    0xFFF
#define GNI_MEMHNDL_SET_MDH(memhndl, value) memhndl.qword1 |= (((uint64_t)value & GNI_MEMHNDL_MDH_MASK) << 52)
#define GNI_MEMHNDL_GET_MDH(memhndl) ((memhndl.qword1 >> 52) & GNI_MEMHNDL_MDH_MASK)
/* Number of Registered pages (1TB for 4kB pages): QWORD2[27:0] */
#define GNI_MEMHNDL_NPGS_MASK   0xFFFFFFF
...
#define GNI_MEMHNDL_PSIZE_MASK  0x3F
...
/* Flags: QWORD2[41:34] */
#define GNI_MEMHNDL_FLAGS_MASK  0xFF
...
#define GNI_MEMHNDL_VERIFY(memhndl) (((memhndl.qword2 >> 56) & 0xFF) == gni_memhndl_calc_crc(&memhndl))
...
/* Use to identify FETCH semantics */
#define GNI_FMA_ATOMIC_NO_FETCH         0x100
/* Use to identify 32 bit AMO requests */
...
/* strings */
#define GNI_CLDEV_MDD_STR	"MDD"
#define GNI_CLDEV_CQ_STR	"CQ"
#define GNI_CLDEV_FMA_STR	"FMA"
#define GNI_CLDEV_RDMA_STR	"RDMA"
#define GNI_CLDEV_DIRECT_STR	"DIRECT"
#if defined CRAY_CONFIG_GHAL_GEMINI
#define GNI_CLDEV_MRT_STR	"MRT"
#define GNI_CLDEV_GART_STR	"GART"
#define GNI_CLDEV_CE_STR	NULL
#define GNI_CLDEV_DLA_STR	NULL
#define GNI_CLDEV_IOMMU_STR	NULL
#elif defined CRAY_CONFIG_GHAL_ARIES
#define GNI_CLDEV_MRT_STR	NULL
#define GNI_CLDEV_GART_STR	NULL
#define GNI_CLDEV_CE_STR	"CE"
#define GNI_CLDEV_DLA_STR	"DLA"
#define GNI_CLDEV_IOMMU_STR	"IOMMU"
#endif

/* indices in this array must match values of gni_dev_res_t in gni_pub.h */
static const char *gni_dev_res_strs[] = {
	NULL,			/* GNI_DEV_RES_FIRST = 0, */
	...
	NULL			/* GNI_DEV_RES_LAST */
};

static inline const char *gni_dev_res_to_str(gni_dev_res_t res) {
	GASSERT(res > GNI_DEV_RES_FIRST && res < GNI_DEV_RES_LAST);
	return gni_dev_res_strs[res];
}

/* indices in this array must match values of gni_job_res_t in gni_pub.h */
static const char *gni_job_res_strs[] = {
	NULL,			/* GNI_JOB_RES_FIRST = 0, */
	GNI_CLDEV_MDD_STR,	/* GNI_JOB_RES_MDD, */
	GNI_CLDEV_MRT_STR,	/* GNI_JOB_RES_MRT, */
	GNI_CLDEV_IOMMU_STR,	/* GNI_JOB_RES_IOMMU, */
	GNI_CLDEV_GART_STR,	/* GNI_JOB_RES_GART, */
	GNI_CLDEV_CQ_STR,	/* GNI_JOB_RES_CQ, */
	GNI_CLDEV_FMA_STR,	/* GNI_JOB_RES_FMA, */
	GNI_CLDEV_RDMA_STR,	/* GNI_JOB_RES_RMDA, */
	GNI_CLDEV_CE_STR,	/* GNI_JOB_RES_CE, */
	GNI_CLDEV_DLA_STR,	/* GNI_JOB_RES_DLA, */
	NULL			/* GNI_JOB_RES_LAST */
};

static inline const char *gni_job_res_to_str(gni_job_res_t res) {
	GASSERT(res > GNI_JOB_RES_FIRST && res < GNI_JOB_RES_LAST);
